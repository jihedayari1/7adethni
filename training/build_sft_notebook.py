#!/usr/bin/env python3
"""Builds training/sft_kaggle.ipynb — Stage 2: SFT on top of the CPT adapter + TunBench eval."""
import json
...
cells.append(md("## 3. Load real SFT + English retention, chat-format"))
cells.append(code(
...
cells.append(md("## 4. Load base 4-bit + the CPT adapter as TRAINABLE (continue the same adapter)"))
cells.append(code(
"import gc, torch\n",
...
cells.append(md("## 5. Mask the prompt, train only on the answer (DataCollator)"))
cells.append(code(
...
cells.append(md("## 6. Save the final adapter"))
cells.append(code(
...
cells.append(md("## 7. TunBench — honest eval (real-word rate + comprehension), BASE vs YOURS"))
cells.append(code(
"import json, re\n",
"model.eval()\n",
...
cells.append(md("## 7b. Number-rule + slang comprehension, and dump predictions for local TunBench"))
cells.append(code(
...
out = Path(__file__).parent / "sft_kaggle.ipynb"
